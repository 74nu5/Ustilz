...
function ProcessVersionTag ($tag) {
    $origVer = $tag."#text"
    $verArray = $tag."#text".Split(".")
...
foreach ($folder in $srcDir) {
    ...
    if (Test-Path $p -PathType Leaf) {
        ...
        $proj.GetElementsByTagName("FileVersion") | ForEach-Object {
            ProcessVersionTag $_
        }
        $proj.GetElementsByTagName("AssemblyVersion") | ForEach-Object {
            ProcessVersionTag $_
        }
    }
}
